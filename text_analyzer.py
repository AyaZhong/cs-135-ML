    ...
    for doc in corpus:
        for word in corpus[doc]:
            if word in corpus_idf:
                ...
                corpus_idf[word] = 1
    #calculate idf use log(n/df)
    for word in corpus_idf:
        corpus_idf[word] = math.log(len(corpus)/corpus_idf[word])
...
def tf_idf(
    ...
    #check if word in sonnet_tf is in corpus_idf
    for i in sonnet_tf:
        if i not in corpus_idf:
            corpus_idf[i] = 0

    for word in sonnet_tf:
...
def cosine_sim(
    ...
    #calculate product of the norms
    norm1 = 0
    for word in vec1:
        norm1 += vec1[word] ** 2
